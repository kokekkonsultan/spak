...
class DataPerolehanSurvei_model extends CI_Model
{
    ...
    var $column_search  = array('waktu_isi');
    ...
    private function _get_datatables_query($table_identity, $profil_responden)
    {
        ...
        $this->db->select("*, id_responden, is_submit, kode_surveyor, responden_$table_identity.uuid AS uuid_responden, waktu_isi, (SELECT first_name FROM u1489187_auth.users WHERE id = surveyor.id_user) AS first_name, (SELECT last_name FROM u1489187_auth.users WHERE id = surveyor.id_user) AS last_name, 
        (SELECT nama_layanan FROM layanan_survei_$table_identity WHERE responden_$table_identity.id_layanan_survei = layanan_survei_$table_identity.id) AS nama_layanan, $query_profil");
        ...
        if ($this->input->post('id_layanan_survei')) {
            $this->db->where('id_layanan_survei', $this->input->post('id_layanan_survei'));
        }
