...
class AuthKlien_model extends CI_Model
{
	...
	{
		$this->db->from('u1489187_auth.users_groups ug');
		$this->db->join('u1489187_auth.groups g', 'g.id = ug.group_id');
		$this->db->join('u1489187_auth.users u', 'u.id = ug.user_id');
		$this->db->join('u1489187_auth.klasifikasi_survei ks', 'u.id_klasifikasi_survei = ks.id');
		$this->db->where('g.name', 'client');
