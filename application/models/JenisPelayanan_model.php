<?php
defined('BASEPATH') or exit('No direct script access allowed');

class JenisPelayanan_model extends CI_Model
{

	var $table 			= 'jenis_pelayanan';
	var $column_order 	= array(null, null, null, null, null);
	var $column_search 	= array('jenis_pelayanan.nama_jenis_pelayanan_responden', 'nama_klasifikasi_survei');
	var $order 			= array('jenis_pelayanan.id' => 'asc');

	public function __construct()
	{
		parent::__construct();
	}

	private function _get_datatables_query()
	{
		$this->db->select('*, jenis_pelayanan.id AS id_jenis_pelayanan');
		$this->db->from('jenis_pelayanan');
		$this->db->join('u1489187_auth.klasifikasi_survei ks', 'ks.id = jenis_pelayanan.id_klasifikasi_survei');

		if ($this->input->post('id_klasifikasi_survei')) {
            $this->db->where('jenis_pelayanan.id_klasifikasi_survei', $this->input->post('id_klasifikasi_survei'));
        }

		$i = 0;

		foreach ($this->column_search as $item) {
			if ($_POST['search']['value']) {

				if ($i === 0) {
					$this->db->group_start();
					$this->db->like($item, $_POST['search']['value']);
				} else {
					$this->db->or_like($item, $_POST['search']['value']);
				}

				if (count($this->column_search) - 1 == $i)
					$this->db->group_end();
			}
			$i++;
		}

		if (isset($_POST['order'])) {
			$this->db->order_by($this->column_order[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
		} else if (isset($this->order)) {
			$order = $this->order;
			$this->db->order_by(key($order), $order[key($order)]);
		}
	}

	function get_datatables()
	{
		$this->_get_datatables_query();
		if ($_POST['length'] != -1)
			$this->db->limit($_POST['length'], $_POST['start']);
		$query = $this->db->get();
		return $query->result();
	}

	function count_filtered()
	{
		$this->_get_datatables_query();
		$query = $this->db->get();
		return $query->num_rows();
	}

	public function count_all()
	{
		$this->db->select('*');
		$this->db->from('jenis_pelayanan');
		$this->db->join('u1489187_auth.klasifikasi_survei ks', 'ks.id = jenis_pelayanan.id_klasifikasi_survei');

		return $this->db->count_all_results();
	}


	public function dropdown_klasifikasi_survei()
	{
		$query = $this->db->get('u1489187_auth.klasifikasi_survei');

		if ($query->num_rows() > 0) {

			$dd[''] = 'Please Select';
			foreach ($query->result_array() as $row) {
				$dd[$row['id']] = $row['nama_klasifikasi_survei'];
			}

			return $dd;
		}
	}
}

/* End of file JenisPelayanan_model.php */
/* Location: ./application/models/JenisPelayanan_model.php */