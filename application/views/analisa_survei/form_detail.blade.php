...
<div class="container-fluid mt-5">
    ...
    <a href="{{ base_url() }}{{ $ci->session->userdata('username'); }}/{{ $ci->uri->segment(2) }}/analisa-survei" title="Kembali" class="btn btn-secondary font-weight-bold">Kembali</a>
    ...
    <div class="card card-custom card-sticky mt-5">
        ...
        <div class="card-body">
            <form id="form-filter-unsur_<?php echo $row->id ?>" class="">
                <div class="row mb-5">
                    <div class="col-md-6">
                        <label for="skor_jawaban_unsur_<?php echo $row->id ?>" class="form-label font-weight-bold text-primary">Filter
                            Berdasarkan
                            Pilihan Jawaban</label>
                        <select id="skor_jawaban_unsur_<?php echo $row->id ?>" class="form-control" onchange="updateUnitUnsur_<?php echo $row->id ?>();">
            ...
            <table id="table_unsur_<?php echo $row->id ?>" class="table table-bordered mt-5" cellspacing="0" width="100%">
                <thead class="">
                    <tr>
                        <th width="5%">No.</th>
...
@foreach($data_analisa->result() as $row)
<div class="modal fade" id="edit{{$row->id_jawaban_pertanyaan_unsur}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?php echo base_url() . $ci->session->userdata('username') . '/' . $ci->uri->segment(2) . '/analisa-survei/edit-alasan'. '/' . $ci->uri->segment(5) ?>"
                    method="POST">

                    <input name="id_jawaban_pertanyaan_unsur" value="{{$row->id_jawaban_pertanyaan_unsur}}" hidden>

                    <div class="form-group">
                        <label class="form-label font-weight-bold">Alasan<span style="color:red;">*</span></label>
                        <textarea id="alasan_pilih_jawaban" name="alasan_pilih_jawaban" class="form-control" required>{{ $row->alasan_pilih_jawaban }}</textarea>
                    </div>
