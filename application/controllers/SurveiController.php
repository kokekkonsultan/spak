...
class SurveiController extends CI_Controller
{
    ...
    public function _get_tamplate()
    {
        ...
        $this->db->join('u1489187_auth.users u', 'manage_survey.id_user = u.id');
    ...
    public function cek_paket()
	{
		...
		$get_parent_induk = $this->db->query("SELECT id_parent_induk FROM u1489187_auth.users WHERE id = $manage_survey->id_user")->row();

		$this->db->select('*, u1489187_auth.berlangganan.id AS id_berlangganan');
		$this->db->from('u1489187_auth.berlangganan');
		$this->db->join('u1489187_auth.users', 'u1489187_auth.users.id = u1489187_auth.berlangganan.id_user');
		$this->db->join('u1489187_auth.paket', 'u1489187_auth.paket.id = u1489187_auth.berlangganan.id_paket');
		$this->db->where('u1489187_auth.berlangganan.id_user', $get_parent_induk->id_parent_induk);
		$this->db->where('u1489187_auth.berlangganan.id_produk', '2');
		$this->db->order_by('u1489187_auth.berlangganan.id', 'asc');
		$get_data = $this->db->get();	
