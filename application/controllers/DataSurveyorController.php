...
class DataSurveyorController extends Client_Controller
{
	...
	public function edit_surveyor($id1, $id2)
	{
		...
		$this->db->from('surveyor');
		$this->db->join('u1489187_auth.users u', 'u.id = surveyor.id_user');
		...
		if (isset($_POST) && !empty($_POST)) {
			...
			if ($this->form_validation->run() === TRUE) {
				...
				$this->db->update('u1489187_auth.users', $data);
	...
	public function delete_surveyor()
	{
		...
		$this->db->delete('u1489187_auth.users', array('id' => $current->id_users));
		$this->db->delete('surveyor', array('id_user' => $current->id_users));
		$this->db->delete('u1489187_auth.users_groups', array('user_id' => $current->id_users));
