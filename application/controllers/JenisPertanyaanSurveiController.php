...
class JenisPertanyaanSurveiController extends CI_Controller{
	...
	public function _get_data_profile($id1, $id2)
	{
		...
		$this->db->join('u1489187_auth.users_groups', 'u1489187_auth.users.id = u1489187_auth.users_groups.user_id');
		$this->db->where('u1489187_auth.users.username', $this->session->userdata('username'));
		$data_user = $this->db->get()->row();
		// $user_identity = 'drs' . $data_user->is_parent;

		$this->db->select('u1489187_auth.users.username, manage_survey.survey_name, is_question, manage_survey.slug, manage_survey.description, manage_survey.is_privacy, manage_survey.table_identity, manage_survey.id_jenis_pelayanan, manage_survey.atribut_pertanyaan_survey');
		...
		// 	$this->db->join("u1489187_auth.users", "supervisor_$user_identity.id_user = u1489187_auth.users.id");
		// }
