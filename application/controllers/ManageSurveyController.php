...
class ManageSurveyController extends Client_Controller
{
    ...
    public function index()
    {
        ...
        $this->db->select('*');
        $this->db->from('u1489187_auth.users u');
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $value) {
            ...
            $klien_user = $this->db->get_where("u1489187_auth.users", array('id' => $value->id_user))->row();
			        ...
			        <div class="d-flex align-items-center">
			            ...
			            <div class="d-flex flex-column flex-grow-1">
			                <div class="row">
			                    ...
			                    <div class="col sm-2 text-right"><div class="text-dark">Nilai IPAK: <b>' .$nilai_indeks. '</b></div>
    ...
    public function add($username)
    {
        ...
        $this->data['form_action'] = base_url() . $this->session->userdata('username') . "/manage-survey/save-survei" . $this->uri->segment(4);
        $this->data['data_user'] = $this->ion_auth->user()->row();
        ...
        $this->data['survey_name'] = [
            ...
            'class'        => 'form-control',
            'required'    => 'required',
            'placeholder' => 'Survei Dinas XXX Kota XXX Tahun XXX, Survei Dinas XXX Periode XXX Sampai XXX',
            'required' => 'required'
        ];
        ...
        $this->data['description'] = [
            ...
            'class'        => 'form-control',
            'rows'         => '3',
            'placeholder' => 'Survei Dinas XXX Kota XXX'
        ];
    ...
    public function create($username)
    {
        ...
        if ($this->input->post('template') == 1) {
            ...
            $skala_likert = 4; //$this->db->get_where('manage_survey', array('id' => $this->input->post('id_manage_survey')))->row()->skala_likert;
        } else {
            $id_jenis_pelayanan = null;
        ...
        $object = [
            ...
            'id_user' => $this->session->userdata('user_id'), //$this->session->userdata('user_id'),
            ...
            'title_header_survey' => serialize(array("SURVEI PERSEPSI ANTI KORUPSI", $this->input->post('organisasi'))),
            ...
            //'deskripsi_opening_survey' => '<p>Bapak/Ibu yang terhormat,<br/><br>Dalam rangka mengukur Indeks Persepsi Anti Korupsi, Bapak/Ibu dipercaya menjadi responden untuk menilai tingkat pelayanan fasilitas umum</p><p>Atas kesediaan Saudara kami sampaikan terima kasih dan penghargaan sedalam-dalamnya.<br/>Jika ada yang perlu dikonfirmasikan terkait survei ini dapat menghubungi WhatsApp +62 895-3362-29033</p><br><p>Hormat kami,</p><b>Tim Survei Indeks Persepsi Anti Korupsi</b>',
            'deskripsi_opening_survey' => '<p>Dalam rangka mengukur Indeks Persepsi Anti Korupsi, Saudara dipercaya menjadi responden pada kegiatan survei ini. Atas kesediaan Saudara memberikan pendapat kami sampaikan terima kasih dan penghargaan sedalam-dalamnya.</p>',

            'deskripsi_konfirmasi_survei' => 'Kuesioner anda sudah diisi, silahkan klik tombol SUBMIT Kuesioner untuk mengakhiri survei.',

            //'deskripsi_selesai_survei' => 'Terima kasih atas kesediaannya dan partisipasinya untuk mengisi kuesioner Survei Persepsi Anti Korupsi.<br>Saran dan penilaian Saudara memberikan konstribusi yang sangat berarti bagi peningkatan instansi kami.',
            'deskripsi_selesai_survei' => 'Terima kasih atas kesediaan Saudara untuk mengisi kuesioner.',

            'template_email_prospek' => '<p>Kami Tim Survei Persepsi Anti Korupsi ${1},</p>
            ...
            'template_whatsapp_prospek' => '<p>Kami Tim Survei Persepsi Anti Korupsi ${1}, memohon kepada Bapak/ Ibu, untuk mengisi Kuesioner ${2} dengan link berikut ini ${3}. Mohon diisi sebelum tanggal ${4}. Atas kesedian dan partisipasinya kami ucapkan Terima Kasih.</p>',
        ...
        if($this->input->post('template') == 3){
            ...
            $this->db->query("CREATE TABLE layanan_survei_cst$insert_id LIKE layanan_survei_$idnt_tabel");
        ...
        $this->db->query("INSERT INTO $tb_definisi_skala SELECT * FROM definisi_skala WHERE skala_likert = IF($skala_likert = 5, 5, 4)");
        ...
        if ($this->input->post('template') == 1) {
			...
			SELECT pertanyaan_terbuka.id, id_unsur_pelayanan, nama_pertanyaan_terbuka, nomor_pertanyaan_terbuka, is_letak_pertanyaan, pertanyaan_terbuka.is_required
            ...
            //CEK KOLOM
            // if ($this->db->field_exists('is_required', 'profil_responden_' . $identitas_tabel))
            // {
                
            // }else{
            //     $this->db->query("ALTER TABLE profil_responden_$identitas_tabel ADD is_required tinyint(1) NULL DEFAULT '1'");
            // }

            $this->db->query("INSERT INTO $tb_profil_responden SELECT * FROM profil_responden_$identitas_tabel");
            $this->db->query("INSERT INTO $tb_kategori_profil_responden SELECT * FROM kategori_profil_responden_$identitas_tabel");
    ...
    public function update_repository()
    {
        ...
        //if($manage_survey->id_sampling != 0){
            ...
            $object['jumlah_populasi'] = $jumlah_populasi;
            $object['jumlah_sampling'] = $sampling;
    ...
    public function _cek_user()
    {
        $this->db->select('*');
        $this->db->from('u1489187_auth.users u');
        $this->db->join('u1489187_auth.users_groups ug', 'u.id = ug.user_id');
    ...
    {
        $this->db->select("*, manage_survey.id AS id_manage_survey, IF(manage_survey.skala_likert = '', 4, manage_survey.skala_likert) AS skala_likert");

        $this->db->from('u1489187_auth.users u');
        $this->db->join('manage_survey', 'manage_survey.id_user = u.id');
        $this->db->join('jenis_pelayanan', 'manage_survey.id_jenis_pelayanan = jenis_pelayanan.id', 'left');
        $this->db->join('u1489187_auth.klasifikasi_survei ks', 'ks.id = u.id_klasifikasi_survei');
