...
class AnalisaSurveiController extends Client_Controller
{
	...
	public function detail_unsur($username, $slug_manage_survey, $id_unsur_pelayanan)
	{
        ...
        $this->db->where("survey_$table_identity.is_submit", 1);
		$this->data['data_analisa'] = $this->db->get();
	...
	public function ajax_list_analisa($username, $slug_manage_survey, $id_unsur_pelayanan)
	{
		...
		foreach ($list as $value) {
			...
			if($value->alasan_pilih_jawaban) {
				$button_edit = '<button type="button" class="btn btn-light-primary btn-sm" data-toggle="modal" data-target="#edit' . $value->id_jawaban_pertanyaan_unsur . '"><i class="fa fa-edit"></i> Edit</button>';
			} else {
				$button_edit = '';
			}
	...
	{
		$table_identity = $this->get_table_identity($this->uri->segment(2));

		$object = [
			'alasan_pilih_jawaban' => $this->input->post('alasan_pilih_jawaban')
		];
		// var_dump($object);
		$this->db->where('id',  $this->input->post('id_jawaban_pertanyaan_unsur'));
		$this->db->update('jawaban_pertanyaan_unsur_' . $table_identity, $object);

		$this->session->set_flashdata('message_success', 'Berhasil mengubah data');
		redirect($this->session->userdata('username') . '/' . $this->uri->segment(2) . '/analisa-survei/detail-unsur/' . $this->uri->segment(5), 'refresh');
	}
