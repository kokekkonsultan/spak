...
class ScanBarcodeController extends Client_Controller
{
	...
	public function process()
	{
		...
		$user = $this->db->get_where('u1489187_auth.users', array('id' => $manage_survey->id_user))->row();
...
<tr>
	...
	public function process2()
	{
		...
		if ($user->foto_profile == NULL) {
			$img = 'http://auth.surveiku/assets/klien/foto_profile/200px.jpg';
		} else {
			$img = 'http://auth.surveiku/assets/klien/foto_profile/' . $user->foto_profile;
		};
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (isset($_POST['with_logo'])) {
				...
				if ($data_user->foto_profile != '') {
					...
					$logopath = "http://auth.surveiku/assets/klien/foto_profile/" . $data_user->foto_profile;
