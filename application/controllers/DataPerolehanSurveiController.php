...
class DataPerolehanSurveiController extends Client_Controller
{
	...
	public function index($id1, $id2)
	{
		...
		$this->data['layanan'] = $this->db->get_where("layanan_survei_$manage_survey->table_identity", array('is_active' => 1));
	...
	public function export()
	{
		...
		$this->data['is_saran'] = $this->data['manage_survey']->is_saran;
		...
		$this->db->select("*, responden_$table_identity.uuid AS uuid_responden, (SELECT first_name FROM u1489187_auth.users u WHERE u.id = surveyor.id_user) AS first_name, (SELECT last_name FROM u1489187_auth.users u WHERE u.id = surveyor.id_user) AS last_name, (SELECT nama_layanan FROM layanan_survei_$table_identity WHERE responden_$table_identity.id_layanan_survei = layanan_survei_$table_identity.id) AS nama_layanan, $query_profil");
		...
		if ((in_array(1, $atribut_pertanyaan)) && ($this->data['manage_survey']->is_saran == 1)) {
			$this->data['colspan'] = (($this->data['unsur']->num_rows() * 3) + $this->data['profil']->num_rows() + 5);
		} else if ((in_array(1, $atribut_pertanyaan)) && ($this->data['manage_survey']->is_saran != 1)) {
