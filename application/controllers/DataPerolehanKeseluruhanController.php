...
class DataPerolehanKeseluruhanController extends CI_Controller
{
	...
	public function ajax_list()
	{
		...
		$users_parent = $this->db->query("SELECT GROUP_CONCAT(id) AS id_parent_induk FROM u1489187_auth.users WHERE id_parent_induk =" . $this->session->userdata('user_id'))->row();
		...
		$data = array();
		foreach ($this->db->query("SELECT *, (SELECT first_name FROM u1489187_auth.users WHERE id = manage_survey.id_user) AS nama_depan, (SELECT last_name FROM u1489187_auth.users WHERE id = manage_survey.id_user) AS nama_belakang FROM manage_survey WHERE id_user IN ($parent)")->result() as $key => $value) {
			...
			$data[$key] = "UNION SELECT
						...
						(SELECT first_name FROM u1489187_auth.users WHERE surveyor.id_user = id),
						(SELECT last_name FROM u1489187_auth.users WHERE surveyor.id_user = id),
						'$value->nama_depan',
						'$value->nama_belakang'
