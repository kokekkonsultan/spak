...
class ProfilRespondenSurveiController extends CI_Controller
{
    ...
    public function index($id1, $id2)
    {
        ...
        // foreach($this->db->get("responden_cst344")->result() as $row){
        //     $data = explode (" ",$row->id_layanan_survei );
        //     // $get = serialize($data);
        //     // var_dump($get);
        //     $object = [
        //         'id_layanan_survei' => serialize($data)
        //     ];
        //     $this->db->where('id', $row->id);
        //     $this->db->update('responden_cst344', $object);

        //     // $id_layanan_survei = serialize($row->id_layanan_survei);
        //     // $this->db->query("UPDATE responden_cst344 SET id_layanan_survei = '$id_layanan_survei' WHERE id = $row->id");
        // }


        // $this->db->query("ALTER TABLE responden CHANGE id_layanan_survei id_layanan_survei TEXT");
        // foreach ($this->db->get("manage_survey")->result() as $row) {
        //     $this->db->query("ALTER TABLE responden_$row->table_identity CHANGE id_layanan_survei id_layanan_survei TEXT");
        //     $this->db->query("ALTER TABLE trash_responden_$row->table_identity CHANGE id_layanan_survei id_layanan_survei TEXT");
        // }
